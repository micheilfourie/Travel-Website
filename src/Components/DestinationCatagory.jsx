...
const DestinationCatagory = () => {
    return (
        <section className="flex flex-col items-center gap-10 max-w-[1200px] mx-auto my-20 p-10 max-sm:px-4 ">
            ...
            <div className="grid grid-cols-6 max-lg:grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 justify-center items-center gap-4 max-sm:gap-0">

                {catagoryList.map(({ image, category, count }, index) => (
                    <a
                        key={index}
                        href=""
                        className="flex flex-col justify-between items-center gap-1 p-4 border-[1px] border-gray-300 border-collapse w-38 h-38 max-sm:w-[90vw] max-sm:h-36"
                    >
